...
filename = paste0(path.data, 'sim_etat_psi.RData')
...
psi = t(chol( kappa_psi * exp( - distmat / phi_psi ) )) %*% rnorm(nG)
...
loglam = foreach(i = 1:nrow(Tw), .combine = cbind) %do% {
  beta0[i] + Xwhale[[i]] %*% beta[1] + beta[2] * (logZ[,indtwhale[i]] - mean(logZ)) + psi
}
trpar$loglam = loglam
lam = exp(loglam)
# range(lam)
